...
If you prefer to install every dependency yourself, see MANUAL_INSTALL.md. 
...
## Usage Instructions
After you pulled the container image, downloaded the pre-built index, and built the alternative humen reference (optional, if a SNP panel is specified, see bottom of this page), follow the steps below to run Sedimix. 
 
1. Create a new folder for your current run (same level as `scripts` and `rules`) and cd into the folder 
...
•	--bind "$(pwd)/..":/workdir mounts the parent directory (containing your run folder and the rules/ folder) into /workdir.
•	--pwd "/workdir/${PWD##*/}" sets the working directory inside the container to your run folder (whatever it’s named).
•	The first snakemake … --unlock clears any stale lock, then the second executes the workflow with your specified resources.
• `cores` and `mem_mb` should equal to number of threads specified in `config.yaml` (e.g. 16)
